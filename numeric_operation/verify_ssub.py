...
def test_sadd(x, y):
    ...
    os.system("fud e std_ssub.futil -s verilog.data data.json --to dat -q --through icarus-verilog > result.json")
    try:
        with open('result.json') as f_2:
            result_data = json.load(f_2)
            s = result_data['memories']['inp'][2]
            if s > 0:
                assert s == (x - y) % (2 ** 31), "positive difference wrong"
            else:
                assert s == (x - y) % (-2 ** 31), "negative difference wrong"
